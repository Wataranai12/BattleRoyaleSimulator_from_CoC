...
class BattleSimulator
  ...
  def execute_attack(attacker, defender)
    ...
    if grapple_condition
      attack_methods = attack_methods.joins(:skill).where(skills: { category: 'grapple' })
    end
    ...
    if is_fumble
      ...
      log_event('ファンブル', "#{char.name} の #{attack_method.show_name}(#{roll}) → ファンブル！ 自分に #{damage}ダメージ、2ラウンド技能-20% (HP: #{attacker.current_hp + damage} → #{new_hp})", character: char)
    ...
    if skill.category == 'grapple'
      ...
      if grapple_condition
        ...
        log_event('組み付き', "#{char.name} の絞め技 → #{defender.character.name} に #{damage}ダメージ (HP: #{defender.current_hp + damage} → #{new_hp})", character: char, target: defender.character)
      ...
      unless defender_grappled
        ...
        # 回避成功
        if dodge_success
          log_event('組み付き', "#{char.name} の #{attack_method.show_name}(#{roll}) → #{defender.character.name} 回避成功(#{dodge_roll})", character: char, target: defender.character)
          return
        end
      end
      ...
      defender.conditions.create!(
        ...
        duration: 99,  # STR対抗で脱出するまで継続
        is_active: true,
        origin_participant_id: attacker.id
      )
      
      log_event('組み付き', "#{char.name} の #{attack_method.show_name}(#{roll}) → #{defender.character.name} を組み付いた！", character: char, target: defender.character)
      return
    end
  ...
  def execute_str_contest(grappled_participant, grappled_condition)
    ...
    if success
      ...
      log_event('STR対抗', "#{grappled_char.name} のSTR対抗(#{roll}/#{success_rate}) → 成功！ 組み付きから脱出", character: grappled_char)
    else
      # 脱出失敗
      log_event('STR対抗', "#{grappled_char.name} のSTR対抗(#{roll}/#{success_rate}) → 失敗、組み付かれたまま", character: grappled_char)
    end
  end
